...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    Page<Message> findConversationBetweenUsersPaginated(@Param("userId1") Long userId1, @Param("userId2") Long userId2, Pageable pageable);
    ...
    @Modifying
    @Query("UPDATE Message m SET m.isRead = true, m.readAt = CURRENT_TIMESTAMP WHERE m.sender.id = :senderId AND m.receiver.id = :receiverId AND m.isRead = false")
